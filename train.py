...
from model import Transcriber, Transcriber_RNN,Transcriber_CRNN, Transcriber_ONF, Transcriber_ONF2, Transcriber_ONF3, Transcriber_ONF4, Transcriber_ONF5,Transcriber_Transformer, Transcriber_Transformer2, Transcriber_Transformer3, Transcriber_Transformer4, Transcriber_Transformer5,Transcriber_Transformer6 
...
def train(model_type, logdir, batch_size, iterations, validation_interval, sequence_length, learning_rate, weight_decay, cnn_unit, fc_unit, debug=False, save_midi=False):
    ...
    elif model_type == 'ONF2':
        model = Transcriber_ONF2(cnn_unit=cnn_unit, fc_unit=fc_unit)
    elif model_type == 'ONF3':
        model = Transcriber_ONF3(cnn_unit=cnn_unit, fc_unit=fc_unit)
    elif model_type == 'ONF4':
        model = Transcriber_ONF4(cnn_unit=cnn_unit, fc_unit=fc_unit)
    elif model_type == 'ONF5':
        model = Transcriber_ONF5(cnn_unit=cnn_unit, fc_unit=fc_unit)
    elif model_type == 'transformer':
        model = Transcriber_Transformer(cnn_unit=cnn_unit, fc_unit=fc_unit)
    elif model_type == 'transformer2':
        model = Transcriber_Transformer2(cnn_unit=cnn_unit, fc_unit=fc_unit)
    elif model_type == 'transformer3':
        model = Transcriber_Transformer3(cnn_unit=cnn_unit, fc_unit=fc_unit)
    elif model_type == 'transformer4':
        model = Transcriber_Transformer4(cnn_unit=cnn_unit, fc_unit=fc_unit)
    elif model_type == 'transformer5':
        model = Transcriber_Transformer5(cnn_unit=cnn_unit, fc_unit=fc_unit)
    elif model_type == 'transformer6':
        model = Transcriber_Transformer6(cnn_unit=cnn_unit, fc_unit=fc_unit)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-b', '--batch_size', default=16, type=int)  # changed to 4 for transformer
